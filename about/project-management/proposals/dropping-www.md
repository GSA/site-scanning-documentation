...
- Possibly the greatest single issue the Site Scanning engine trips up on is following meta redirects.  The use of these client-side redirects very often causes timeout issues that cause a scan to fail. This is a longstanding issue that we've not been able to overcome in several years of trying.  At the same time as this experimental dataset was generated, another experimental scan is in the works to try and detect meta-redirects.  It's a work in progress and appears to be incomplete at this point, but the results can be seen in the `client_redirect_scan_status` and `has_client_redirect` fields and are also in this snapshot.
- Unlike virtually every other scan error message, `execution_context_destroyed` is more of a puppeteer error message than an server response message, so I'm keeping an eye on any results that have those results to see if they are false negatives in this analysis.  There's only a few, but I've found and factored in a few so far.  
...
- This problem is not new and [has been rightly called out for a good while now](https://web.archive.org/web/20120722183122/http://techpresident.com/blog-entry/todays-rant-death-mandatory-dub-dub-dubs).  
